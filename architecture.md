...
前端是 Vite + React + Mantine + TanStack Query，同一份构建同时服务 Web 和 Tauri 2 客户端。后端是 FastAPI + pandas + AkShare；桌面生产包通过 PyInstaller sidecar 内置后端，Web 则继续使用独立 API 服务。学习记忆和策略实验进入 SQLite/Postgres 数据库，行情快照和报告仍作为可重建缓存保存。
...
| 桌面入口 | `desktop/sidecar.py` | 启动打包后的 FastAPI/uvicorn 服务 |
...
- 主窗口与悬浮窗关闭时进入隐藏状态，应用和 sidecar 继续服务；托盘退出统一回收 sidecar。
...
- 配置 `STOCK_LAB_DATABASE_URL=postgresql://...`：使用 Postgres，适合多实例部署和长期数据存储。
...
- 外部 Postgres 仍需要容量监控和定期备份。
- 行情原始缓存和报告仍在实例文件系统中，容器或实例替换后可能丢失；它们可重建，但学习库必须持久化。

## 9. 部署架构

推荐使用单个 Docker 服务同时承载 Web 静态文件和 FastAPI：

```mermaid
flowchart LR
  C["Git main"] --> B["CI build"]
  B --> R["Docker runtime"]
  U["Browser / Tauri"] --> R
  R --> F["FastAPI API"]
  R --> W["frontend/dist static files"]
  F --> D["行情与报告缓存"]
  F --> P["SQLite / Postgres"]
  F --> M["Public market-data providers"]
```
...
- 单个镜像可以锁定前端和后端版本，并在发布后执行健康检查。
- 运行平台负责计算，持久化卷或外部 Postgres 保存长期数据。

部署边界：

- `STOCK_LAB_DATA_DIR` 默认是 `/tmp/stock-opportunity-lab`，不能作为长期存储。
- 长期学习记录、用户设置和策略实验应通过持久化卷或 `STOCK_LAB_DATABASE_URL` 保存。

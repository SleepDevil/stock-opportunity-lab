            ...
            self._json(
                {
                    "subscriptions": [],
                    "articles": [],
                    "capability_note": "当前静态镜像只提供轻后端；公众号知识写入需要自行部署完整后端。",
                }
            )
            return
        self._unavailable()
        ...
        self._json(
            {
                ...
                "detail": (
                    "当前静态镜像只提供轻后端：支持前端、健康检查、配置和只读学习摘要。"
                    "用户设置、公众号写入、盘后扫描、实时行情和财务采集需要自行部署完整后端。"
                )
            },
...
def empty_learning_summary() -> dict[str, object]:
    return {
        ...
        "strategy_insights": {
            "status": "collecting",
            "message": "静态镜像轻后端只提供只读占位摘要；真实学习库需要自行部署完整后端。",
            "recommendations": [],
        },
        "recent_records": [],
        "updated_at": None,

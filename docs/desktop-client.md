...
Tauri WebView -> bundled React assets
                ...
                -> shared HTTPS sync API (account/watchlist/commentary only)
```

客户端启动时会检查 `/api/health`，sidecar 准备好后才挂载完整路由；API 不可达时会进入明确的错误态并允许重新连接，不会无限显示 loading。账户、自选和锐评请求使用 `VITE_STOCK_LAB_SYNC_API_BASE_URL` 指定的共享服务；官方构建默认连接项目平台地址。定制构建仍可设置 `VITE_STOCK_LAB_API_BASE_URL` 整体改用自建 HTTPS API，这些地址会进入前端 Bundle，因此不能包含密钥。
...
- `widget`：可拖拽、可缩放的行情悬浮窗，启动时默认隐藏；从主界面或系统托盘打开后，窗口置顶并停靠在当前屏幕右上角。

悬浮窗使用 `/desktop-widget` 轻量路由，包含“自选行情”和“今日机会”视图。“自选行情”顶部是可切换的主行情卡，默认显示上证指数最新点位、涨跌幅、沪深两市当日成交额和分时走势；在卡片上单击右键，可改为显示任一当前自选股。新增、删除和调整自选顺序会先保存到本地，再自动同步到与 Web 共用的服务端名单；首次升级会合并两端数据，避免覆盖客户端尚未上传的股票。标题栏云朵图标展示同步中、已同步、缺少账户或同步失败状态。股票模式显示该股当日成交额与分时走势，同时上证指数会与该股票交换到原来的列表位置，确保指数和每只自选股都只出现一次；被选中的股票移出自选后自动回退到上证指数。标题栏空白区域支持拖拽移动窗口，窗口四边和四角支持原生缩放，最小尺寸为 `320 × 360`；移动和缩放后的状态会被保存。刷新、置顶切换、隐藏由右侧图标按钮控制；悬浮窗隐藏后，可从主界面右上角的“悬浮窗”按钮或系统托盘重新打开。
...
- 点击悬浮窗右上角 `+`，可按名称、代码或拼音首字母添加股票；自选名单不设数量上限。
...
- 自选列表先保存在客户端本地，随后自动写入服务端；重启、切换 Web/桌面或暂时断网都不会丢失。同步失败时标题栏云朵会变为错误状态，可点击重试。
- 实时行情源暂时不可用时会保留最近本地快照并明确标注，不会一直停留在加载状态。

### 走势锐评

- 服务端在 A 股交易日的 `10:00`、`11:30`、`15:00` 自动生成并推送锐评；行情本身仍按 15 秒节奏刷新。也可在锐评页或策略设置页点击“立即锐评并推送”，强制刷新当前自选与大盘行情，并在闭市后发送标记为“手动触发”的最新可用快照。
- 每次只提交当前自选的价格、涨跌、成交、换手和上证指数快照，并附带程序计算出的红盘/绿盘数量、平均涨跌、领涨与领跌；提示词禁止补写新闻、涨跌原因、未来走势或买卖指令。
- 同一时段按“时段 + 自选成员”去重。拖拽排序不会重复调用 AI，增删股票后会对新名单重新生成；最新结果保存在客户端本地，午休、收盘或重启后仍可查看当天最近一条。
- 配置服务端 `STOCK_LAB_ZHIPU_API_KEY` 后，默认调用智谱免费模型 `glm-4.7-flash` 生成风趣短评；请求使用 JSON 结构化输出、关闭深度思考以降低延迟，并要求覆盖全部自选名称。密钥未配置、超时、限流或响应校验失败时明确显示“规则代笔”，并用同一份结构化行情生成可用兜底，不会伪装成 AI 输出。旧的 `STOCK_LAB_AI_COMMAND` 仍可通过 `STOCK_LAB_AI_PROVIDER=command` 使用。
- 打包客户端从 Tauri 应用数据目录下的 `.env` 读取智谱与飞书服务端密钥；该文件不进入前端 Bundle 或安装包，建议仅授予当前用户读写权限。进程环境变量具有更高优先级，便于企业部署覆盖本地配置。
- 在“策略设置 → 自选锐评群订阅”中可单独开启飞书群推送，并配置目标群 `chat_id` 与群成员可访问的平台根地址。平台地址用于生成 `/stock?symbol=股票代码` 深链，不应填写 `localhost` 或只在本机可用的 Tauri 地址。
- 开关开启后，连续交易时段每次锐评成功生成都会发送 Card 2.0 卡片；卡片包含组合均值、涨跌数量、锐评正文、全部自选明细和风险说明。正文与明细里的股票名均为超链接，点击后进入个股分析页，自动填入代码并直接加载当日分析。
- 飞书应用仍通过 `STOCK_LAB_FEISHU_APP_ID` 和 `STOCK_LAB_FEISHU_APP_SECRET` 配置；机器人必须加入目标群并具备 `im:message` 发送消息权限。设置页提供“发送卡片测试”，用于验证应用凭证、群 ID、机器人入群状态与个股链接。
- 页签和标题栏刷新按钮都支持手动重评；非连续交易时段的手动锐评只在悬浮窗展示，不发送群卡片。所有锐评均保留“仅复述行情快照、不构成投资建议”的边界说明。
...
- Web 与 Tauri 通过 `/api/client-auth` 自动获取短时签名 token，不要求用户输入密钥。扫描和报告接口发送 `X-Stock-Lab-CSRF`；自选同步、通知设置和锐评接口不依赖报告鉴权。
- `/api/client-auth` 保持兼容旧版 Tauri；服务端签名密钥不写入安装包或前端构建产物。群 ID 只保存在服务端用户配置中，不随每次行情快照回传到客户端。
...
```bash
npm run test:api       # FastAPI/业务测试，含客户端 token 与 Tauri 兼容契约

...
LEARNING_READ_TIMEOUT_SECONDS = 3.0
...
def load_learning_summary_with_timeout(
    config: AppConfig,
    limit: int = 20,
    timeout_seconds: float = LEARNING_READ_TIMEOUT_SECONDS,
) -> dict[str, Any]:
    return call_with_timeout(lambda: load_learning_summary(config, limit=limit), timeout_seconds)
...
def read_learning_records_with_timeout(
    config: AppConfig,
    timeout_seconds: float = LEARNING_READ_TIMEOUT_SECONDS,
) -> dict[str, dict[str, Any]]:
    return call_with_timeout(lambda: read_learning_records(config), timeout_seconds)


def call_with_timeout(fn: Callable[[], Any], timeout_seconds: float) -> Any:
    ...
    try:
        ...
        raise TimeoutError(f"learning store read exceeded {timeout_seconds:g}s") from exc
    if ok:
        return value
    raise value
...
def annotate_candidates_with_learning(
    config: AppConfig,
    candidates: pd.DataFrame,
    records: list[dict[str, Any]] | None = None,
) -> pd.DataFrame:
    records = records if records is not None else list(read_learning_records(config).values())
    out = candidates.copy()
    if out.empty:

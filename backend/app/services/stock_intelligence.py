...
from app.services.data_provider import available_spot_snapshot_dates, load_spot_cache_frame, normalize_stock_code
...
class StockIntelligenceProvider(Protocol):
    ...
    def stock_name(self, symbol: str, trade_date: str | None = None) -> str:
        ...
...
class AkShareStockIntelligenceProvider:
    ...
    _stock_name_memory: dict[str, str] = field(default_factory=dict, init=False, repr=False)
    ...
    def stock_name(self, symbol: str, trade_date: str | None = None) -> str:
        ...
        if code in self._stock_name_memory:
            return self._stock_name_memory[code]

        name = cached_stock_name(self.config, code, trade_date=trade_date)
        if not name:
            name = akshare_stock_name(self._ak, code)
        if name:
            self._stock_name_memory[code] = name
        return name
...
def run_stock_intelligence(
    ...
    normalized_start_date = normalize_trade_date(start_date) if start_date else normalized_date
    if normalized_start_date > normalized_date:
        normalized_start_date, normalized_date = normalized_date, normalized_start_date
    notice_start = normalized_start_date
    ...
    search_keywords = news_search_keywords(provider, notices, code, normalized_date)
    search_frames = [
        safe_frame(lambda keyword=keyword: provider.news_search(keyword, page_size=50))
    ...
    dragon_tiger = dragon_tiger_payload(provider, code, normalized_start_date, normalized_date)

    return {
        "code": code,
        "trade_date": normalized_date,
        "query_start_date": normalized_start_date,
        "query_end_date": normalized_date,
...
def dragon_tiger_payload(
    ...
    normalized_end = normalize_trade_date(end_date) if end_date else normalized_start
    if normalized_start > normalized_end:
        normalized_start, normalized_end = normalized_end, normalized_start
    available_dates = lhb_available_dates(safe_frame(lambda: provider.lhb_dates(code)))
    lhb_date = next((date for date in available_dates if normalized_start <= date <= normalized_end), None)
    if lhb_date is None:
        return {
            "query_start_date": normalized_start,
            "query_end_date": normalized_end,
...
def news_search_keyword(notices: list[dict[str, Any]], code: str) -> str:
    keywords = news_search_keywords(None, notices, code)
    return keywords[0] if keywords else code


def news_search_keywords(
    provider: StockIntelligenceProvider | None,
    ...
    for notice in notices:
        add_unique_text(keywords, notice.get("name"))
    if provider is not None:
        add_unique_text(keywords, provider_stock_name(provider, code, trade_date))
    add_unique_text(keywords, normalize_stock_code(code))
    return keywords or [normalize_stock_code(code)]


def provider_stock_name(
    provider: StockIntelligenceProvider,
    code: str,
    trade_date: str | None = None,
) -> str:
    loader = getattr(provider, "stock_name", None)
    if not callable(loader):
        return ""
    try:
        return text_value(loader(code, trade_date=trade_date))
    except TypeError:
        try:
            return text_value(loader(code))
        except Exception:
            return ""
    except Exception:
        return ""
...
def cached_stock_name(config: AppConfig, symbol: str, trade_date: str | None = None) -> str:
    code = normalize_stock_code(symbol)
    target = normalize_trade_date(trade_date)
    dates = available_spot_snapshot_dates(config.raw_dir)
    candidates = [target]
    ...
    for snapshot_date in candidates:
        ...
        cache = config.raw_dir / f"spot_{snapshot_date}.csv"
        frame = load_spot_cache_frame(cache)
        if frame is None or "名称" not in frame.columns:
            continue
        rows = frame[frame["代码"].astype(str).str.zfill(6).eq(code)]
        if rows.empty:
            continue
        name = text_value(rows.iloc[0].get("名称"))
        if name:
            return name
    return ""


def akshare_stock_name(ak_loader: Callable[[], Any], symbol: str) -> str:
    code = normalize_stock_code(symbol)
    try:
        with quiet_akshare_output():
            frame = ak_loader().stock_individual_info_em(symbol=code)
    except Exception:
        return ""
    if not isinstance(frame, pd.DataFrame) or frame.empty:
        return ""
    for _, row in frame.iterrows():
        item = text_value(first_existing(row, "item", "项目", "指标"))
        if item not in {"股票简称", "股票名称", "证券简称", "名称"}:
            continue
        name = text_value(first_existing(row, "value", "值", "内容"))
        if name:
            return name
    return ""

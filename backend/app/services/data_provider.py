...
class AkShareProvider:
    ...
    def spot(self, trade_date: str, refresh: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            for attempt in range(3):
                ...
                stale = load_spot_cache_frame(cache)
                if stale is None:
    ...
    def legacy_spot_snapshot(self, trade_date: str) -> pd.DataFrame:
        raw = self._ak().stock_zh_a_spot()
        if raw.empty:
            return empty_spot_frame()
        reference = load_latest_spot_reference(self.config, trade_date)
        reference_by_code = {}
        if not reference.empty and "代码" in reference.columns:
            reference = normalize_rebuilt_spot_frame(reference)
            reference_by_code = {
                normalize_stock_code(str(row.get("代码", ""))): row
        ...
        for index, (_, row) in enumerate(raw.iterrows(), start=1):
            ...
            ref_latest = safe_float(ref.get("最新价")) if ref is not None else math.nan
            ref_total_cap = ref.get("总市值") if ref is not None else math.nan
            ref_float_cap = ref.get("流通市值") if ref is not None else math.nan
            total_cap = scale_cap_to_close(ref_total_cap, ref_latest, latest)
            float_cap = scale_cap_to_close(ref_float_cap, ref_latest, latest)
            volume_shares = safe_float(row.get("成交量"))
            turnover = estimate_turnover(volume_shares, ref_float_cap, ref_latest)
            if not math.isfinite(turnover) and ref is not None:
                turnover = safe_float(ref.get("换手率"))
            previous_close = safe_float(row.get("昨收"))
            high = safe_float(row.get("最高"))
            low = safe_float(row.get("最低"))
            amplitude = calc_amplitude(high, low, previous_close)
            rows.append(
                {
                    "序号": index,
                    "代码": code,
                    ...
                    "成交量": volume_shares / 100 if math.isfinite(volume_shares) else math.nan,
                    ...
                    "昨收": previous_close,
                    "量比": safe_float(ref.get("量比")) if ref is not None else math.nan,
                    "换手率": turnover,
                    "市盈率-动态": ref.get("市盈率-动态") if ref is not None else math.nan,
                    "市净率": ref.get("市净率") if ref is not None else math.nan,
                    "总市值": total_cap,
                    "流通市值": float_cap,
                    "涨速": ref.get("涨速") if ref is not None else math.nan,
                    "5分钟涨跌": ref.get("5分钟涨跌") if ref is not None else math.nan,
                    "60日涨跌幅": ref.get("60日涨跌幅") if ref is not None else math.nan,
                    "年初至今涨跌幅": ref.get("年初至今涨跌幅") if ref is not None else math.nan,
                }
            )
        if not rows:
            return empty_spot_frame()
        return normalize_rebuilt_spot_frame(pd.DataFrame(rows))
    ...
    def index_history(
        ...
        self.config.ensure_dirs()
        code = str(symbol).strip().lower() or "sh000001"
        start = normalize_trade_date(start_date)
        end = normalize_trade_date(end_date)
        cache = self.config.history_dir / f"index_{code}_{start}_{end}.csv"
        if cache.exists() and not refresh:
            cached = pd.read_csv(cache, dtype={"股票代码": str})
            if history_cache_usable(cached, start, end):
                return cached
        ...
        raw = self._ak().stock_zh_index_daily(symbol=code)
        frame = normalize_history_frame(raw, code)
        if not frame.empty:
            date_keys = pd.to_datetime(frame["日期"], errors="coerce").dt.strftime("%Y%m%d")
            frame = frame[(date_keys >= start) & (date_keys <= end)].reset_index(drop=True)
    ...
    def intraday(
        ...
        if cache.exists() and not refresh:
            ...
            if cached.empty and not raw_cached.empty:
                cache.unlink(missing_ok=True)
            elif should_use_intraday_cache(date_key, cache, cached):
                return cached
        if not refresh and date_key != date.today().strftime("%Y%m%d"):
            empty = empty_intraday_frame()
            empty.to_csv(cache, index=False, encoding="utf-8-sig")
            return empty
    ...
    def _load_intraday_em(self, symbol: str, period: str, trade_date: str, adjust: str) -> pd.DataFrame:
        ...
        start = f"{day[:4]}-{day[4:6]}-{day[6:]} 09:15:00"
...
CURRENT_SPOT_CACHE_TTL_SECONDS = 60
CURRENT_INTRADAY_CACHE_TTL_SECONDS = 60
...
def should_use_intraday_cache(
    trade_date: str,
    cache: Path,
    cached: pd.DataFrame | None = None,
    now: datetime | None = None,
    intraday_ttl_seconds: int = CURRENT_INTRADAY_CACHE_TTL_SECONDS,
) -> bool:
    if not cache.exists():
        return False
    current = now or datetime.now()
    ...
    if current >= close_cutoff:
        if cache_mtime < close_cutoff:
            return False
        return intraday_frame_has_close_bar(cached, normalized)

    age_seconds = (current - cache_mtime).total_seconds()
    return 0 <= age_seconds <= intraday_ttl_seconds


def intraday_frame_has_close_bar(df: pd.DataFrame | None, trade_date: str) -> bool:
    if df is None or df.empty or "时间" not in df.columns:
        return False
    normalized = normalize_trade_date(trade_date)
    filtered = filter_intraday_trade_date(df, normalized)
    if filtered.empty:
        return False
    parsed = pd.to_datetime(filtered["时间"], errors="coerce")
    if parsed.isna().all():
        return False
    last = parsed.max()
    return last.hour * 60 + last.minute >= 15 * 60


def load_spot_cache_frame(cache: Path) -> pd.DataFrame | None:
    if not cache.exists():
        return None
    try:
        df = pd.read_csv(cache, dtype={"代码": str})
...
def load_latest_spot_reference(config: AppConfig, trade_date: str) -> pd.DataFrame:
    target = normalize_trade_date(trade_date)
    available = available_spot_snapshot_dates(config.raw_dir)
    if not available:
        return empty_spot_frame()
    preferred = max((value for value in available if value <= target), default=max(available))
    cache = config.raw_dir / f"spot_{preferred}.csv"
    frame = load_spot_cache_frame(cache)
    return frame if frame is not None else empty_spot_frame()
...
def estimate_turnover(volume_shares: Any, float_market_cap: Any, reference_price: Any) -> float:
    volume = safe_float(volume_shares)
    cap = safe_float(float_market_cap)
    price = safe_float(reference_price)
    if not math.isfinite(volume) or not math.isfinite(cap) or not math.isfinite(price) or cap <= 0 or price <= 0:
        return math.nan
    float_shares = cap / price
    if float_shares <= 0:
        return math.nan
    return volume / float_shares * 100


def calc_amplitude(high: Any, low: Any, previous_close: Any) -> float:
    high_value = safe_float(high)
    low_value = safe_float(low)
    previous = safe_float(previous_close)
    if not math.isfinite(high_value) or not math.isfinite(low_value) or not math.isfinite(previous) or previous <= 0:
        return math.nan
    return (high_value - low_value) / previous * 100

...
def require_client_auth(request: Request, config: AppConfig) -> None:
    ...
    desktop_request = request_origin in DESKTOP_FRONTEND_ORIGINS
    if not csrf_header:
        raise ClientAuthError("缺少客户端鉴权令牌")
    if not desktop_request and (not csrf_cookie or not hmac.compare_digest(csrf_header, csrf_cookie)):
        raise ClientAuthError("缺少客户端鉴权令牌")
...
def reject_untrusted_origin_if_present(request: Request) -> None:
    candidate = request_client_origin(request)
    if candidate and not is_trusted_origin(candidate, request):
        raise ClientAuthError("请求来源不可信")

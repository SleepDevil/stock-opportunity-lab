...
from app.services.learning import annotate_candidates_with_learning, read_learning_records_with_timeout
...
def run_screen(
    ...
    if raw.attrs.get("stock_lab_cache_fallback"):
        report(12, "实时行情源暂不可用，已使用同日本地快照继续扫描。")
    elif raw.attrs.get("stock_lab_legacy_spot_fallback"):
    ...
    report(52, "评分排序完成，读取策略学习记录。")
    learning_records = screen_learning_records(config, progress=progress)
    report(56, f"策略学习记录读取完成：{len(learning_records)} 条。")
    target_pool = prepare_monitor_pool(ranked, config, learning_records=learning_records, progress=progress)
    ...
    report(64, f"生成 Top {len(candidates)} 候选价格计划。")
    ...
    report(92, "写入本地 CSV/JSON/Markdown 报告。")
...
def screen_learning_records(config: AppConfig, progress: ProgressCallback | None = None) -> list[dict[str, Any]]:
    try:
        return list(read_learning_records_with_timeout(config).values())
    except Exception as exc:
        if progress:
            progress(56, f"策略学习记录读取失败，已跳过学习提示：{type(exc).__name__}")
        return []
...
def enrich_candidates(
    ...
    total = len(out)
    for index, code in enumerate(out["代码"].astype(str), start=1):
        ...
        if progress and (index == total or index % 5 == 0):
            progress(62 + round(index / max(total, 1) * 8), f"补行业信息 {index}/{total}：{code}")
    out["行业"] = industries
...
def attach_trend_points(
    ...
    for index, code in enumerate(out["代码"].astype(str), start=1):
        try:
            ...
            trends.append([])
            if progress:
                progress(70 + round(index / total * 20), f"拉取候选走势图 {index}/{total}：{code} 失败，继续下一只。")
            continue
        trends.append(history_to_trend_points(history, days=days))
        if progress:
            progress(70 + round(index / total * 20), f"拉取候选走势图 {index}/{total}：{code}")
    out["走势点位"] = trends
    return out
...
def prepare_monitor_pool(
    ...
    if progress:
        progress(58, f"生成监控池价格计划：{len(targets)} 只。")
    targets = attach_buy_plan(targets, config.strategy)
    if progress:
        progress(60, f"为监控池匹配学习样本：{len(targets)} 只。")
    targets = annotate_candidates_with_learning(config, targets, records=learning_records)
    if progress:
        progress(62, "监控池学习提示已生成。")

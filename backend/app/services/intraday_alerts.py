...
def collect_spot_alerts_with_snapshot(
    ...
    if "代码" not in spot.columns:
        return []
    snapshot_label = snapshot_label_from_attrs(spot, trade_date)
    spot = spot.copy()
    ...
    for _, candidate in candidates.iterrows():
        ...
        if code not in spot_by_code.index:
            ...
            continue
        alerts.extend(
            build_candidate_alerts_from_spot(
                candidate,
                spot_by_code.loc[code],
                trade_date,
...
def load_alert_spot_snapshot(
    ...
    if not refresh and normalized == today and cache.exists() and not should_use_spot_cache(normalized, cache):
        try:
            return attach_snapshot_attrs(provider.spot(normalized, refresh=True), snapshot_time=datetime.now())
        except Exception as exc:
            cached = pd.read_csv(cache, dtype={"代码": str})
            return attach_snapshot_attrs(
                cached,
                snapshot_time=datetime.fromtimestamp(cache.stat().st_mtime),
                stale=True,
                reason=str(exc),
            )
    spot = provider.spot(normalized, refresh=refresh)
    if normalized == today:
        snapshot_time = datetime.now() if refresh or not cache.exists() else datetime.fromtimestamp(cache.stat().st_mtime)
        return attach_snapshot_attrs(spot, snapshot_time=snapshot_time)
    return spot


def attach_snapshot_attrs(
    spot: pd.DataFrame,
    snapshot_time: datetime,
    stale: bool = False,
    reason: str | None = None,
) -> pd.DataFrame:
    spot.attrs["stock_lab_snapshot_time"] = snapshot_time.replace(microsecond=0).isoformat()
    spot.attrs["stock_lab_snapshot_stale"] = stale
    if reason:
        spot.attrs["stock_lab_snapshot_reason"] = reason
    return spot


def snapshot_label_from_attrs(spot: pd.DataFrame, trade_date: str) -> str | None:
    snapshot_time = spot.attrs.get("stock_lab_snapshot_time")
    if not isinstance(snapshot_time, str) or not snapshot_time:
        return None
    try:
        parsed = datetime.fromisoformat(snapshot_time)
    except ValueError:
        return f"{display_date(trade_date)} {snapshot_time} 快照"
    suffix = "缓存快照" if spot.attrs.get("stock_lab_snapshot_stale") else "快照"
    return f"{display_date(trade_date)} {parsed.strftime('%H:%M:%S')} {suffix}"
...
def build_candidate_alerts_from_spot(
    candidate: pd.Series,
    spot: pd.Series,
    trade_date: str,
    ...
    latest_time = snapshot_label or f"{display_date(trade_date)} 快照"
